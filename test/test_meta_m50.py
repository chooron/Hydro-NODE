...
from models.NODE_models_Batch import ODESolver, M50_Func
from torch.utils.data import DataLoader, random_split
from sklearn.metrics import r2_score, mean_squared_error

from models.common_net import M50_NN
...
if use_pretrain:
    ...
    et_pretrained_model = M50_NN(input_dim=3, output_dim=1, hidden_units=16,
                                 means=means[[0, 1, 3]], stds=stds[[0, 1, 3]]).to(device)
    q_pretrained_model = M50_NN(input_dim=2, output_dim=1, hidden_units=16,
                                means=means[[1, 2]], stds=stds[[1, 2]]).to(device)
...
m50_func = M50_Func(ET_net=et_pretrained_model, Q_net=q_pretrained_model, ode_lib=solver_lib,
                    params=(S0, S1, f, Smax, Qmax, Df, Tmax, Tmin),
                    interps=(precp_spline, temp_spline, lday_spline))
# 3.train the model based on pytorch-lightning
optimizer = torch.optim.AdamW(m50_func.parameters(), lr=0.001)
...
m50_trained_model, m50_trained_learner = train(
    ...
    os.path.join(save_path, str(basin_id), 'train', 'M50-Batch', 'MSE'),
